...
class PolicyGradient:
    ...
    def _compute_one_policy_entropy_err(self, pi, a):
        ...
        # Add normalization constant to avoid zero in log().
        ...
        policy_err = self.policy_loss(logpi, a.detach().cpu())
    ...
    def feed(self, Q, pi_s, actions, stats, old_pi_s=dict()):
        ...
        for pi_node, a_node in self.policy_action_nodes:
            ...
            if pi_node in old_pi_s:
                ...
                pi = pi.to("cpu")
                old_pi = old_pi.to("cpu")

                # Cap it.
                coeff = torch.clamp(pi.data.div(old_pi), max=args.ratio_clamp).gather(1, a.view(-1, 1).detach().cpu()).squeeze()
                pg_weights.mul_(coeff.detach().cuda())
