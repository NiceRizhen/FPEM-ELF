...
class ActorCritic:
    ...
    # 这里的update是更新模型参数的入口，可以修改本文件，自定义实现自己的Model
    def update(self, mi, batch, stats, cur_policy=None):
        ...
        for t in range(T - 2, -1, -1):
            ...
            # state_curr 是从我们的模将bht(state)输入我们的模型之后返回的数据
            # state_curr(dict): h v pi weight_pi action_type
            state_curr = m.forward(bht, cur_policy)
        ...
        w = torch.ones_like(err, requires_grad=True)
        err = err + w
        err.backward()
