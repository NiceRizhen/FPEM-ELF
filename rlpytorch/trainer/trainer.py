...
class Evaluator:
    ...
    def actor(self, batch, cur_policy=None):
        ''' Actor.
        ...
        # 把batch传给actor(rts/game_mc/model.forward), 从中得到state_curr
        state_curr = m.forward(batch.hist(0), cur_policy)
        ...
        # 自定义的weight prob 返回被存储在state_curr中
        if "weight_pi" in self.keys_in_reply:
            reply_msg["weight_pi"] = state_curr["weight_pi"].data
...
class Trainer:
    ...
    def actor(self, batch, cur_policy=None):
        ...
        return self.evaluator.actor(batch, cur_policy)

    def train(self, batch, cur_policy=None):
        ''' Trainer.
        ...
        mi.zero_grad()
        self.rl_method.update(mi, batch, self.counter.stats, cur_policy)
        mi.update_weights()
        ...
        if self.counter.counts["train"] % self.args.freq_update == 0:
            ...
            mi.update_model("actor", mi["model"])  # 这里的更新不是更新模型参数，而是将actor model换成最新的model
    ...
    def episode_summary(self, i, cur_policy=None):
        ...
        if cur_policy is not None:
            print('================Iteration: {}================'.format(cur_policy))
        prefix = "[%s][%d] Iter" % (str(datetime.now()), args.batchsize) + "[%d]: " % i
        print(prefix)

        # save_model
        if self.counter.counts["train"] > 0:
            self.saver.feed(self.evaluator.mi["model"], cur_policy)
