...
class ModelSaver:
    ...
    def feed(self, model, cur_policy=9):
        args = self.args
        basename = args.save_prefix + "-%d.bin" % cur_policy
...
class ValueStats:
    ...
    def record_data(self, file, content):

        with open(file, 'a+') as f:
            f.write('{0}\n'.format(content))

    # a summary function for recording
    def summary(self, info=None):
        ...
        if self.counter > 0:
            try:
                ret_str = "%s%s[%d]: avg: %.5f, min: %.5f[%d], max: %.5f[%d]" \
                        % (info, name, self.counter, self.summation / self.counter, self.min_value, self.min_idx, self.max_value, self.max_idx)

                if ':reward' in ret_str:
                    self.record_data('record/reward.txt', ret_str)

                self.record_data('record/fpem.txt', ret_str)
                return ret_str
