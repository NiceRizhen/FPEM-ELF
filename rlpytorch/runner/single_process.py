...
class SingleProcessRun:
    ...
    # time for fpem algo running!
    def fpem_run(self):

        self.GC.Start()
        args = self.args

        for cur_policy_n in range(args.max_policy):
            for k in range(args.num_episode):
                ...
                for i in iterator:
                    self.GC.Run(cur_policy_n)

                if self.episode_summary is not None:
                    self.episode_summary(k, cur_policy_n)
