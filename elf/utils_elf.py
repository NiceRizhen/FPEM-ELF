...
class Batch:
    ...
    # 这里直接把batch中的V取出来，相当于取batch[key]
    ...
    def copy_from(self, src, batch_key=""):
        ...
        if "weight_pi" in this_src.keys():
            self.add("weight_pi", this_src["weight_pi"])

        if "bp" in this_src.keys():
            self.add("bp", this_src["bp"])
...
class GCWrapper:
    ...
    def _call(self, infos,cur_policy=None):
        ...
        reply = self._cb[infos.gid](picked,cur_policy)
    ...
    def Run(self,cur_policy=None):
        ...
        res = self._call(self.infos, cur_policy)
        # print("before_step")
