...
class BatchStats:
    ...
    def add_stats(self, batch):
        for agent_name in batch[b"_agent_name"]:
            self.stats_agent[agent_name] += 1

        self.seq_stats.feed(batch[b"_seq"])
...
class MemoryReceiver:
    ...
    def _get_key(self, data):
        return "%s-%d-%d" % (data[b'_agent_name'], data[b"_game_counter"], data[b"_seq"])
